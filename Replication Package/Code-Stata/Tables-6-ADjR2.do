		...
		display "`opv'"
		 if `opv'>20{
			...
			*simple correlation	
			...
			corr exportrank`i' exportrank`j'
		
			local   A`i'_`j'  : di %3.2f int(100*r(rho))/100
					
			*adjusted correalatio
			reg  exportrank`i' exportrank`j' i.nprod
			local R_full=e(r2_a)
		
			reg  exportrank`i'  i.nprod

			local R_minus=e(r2_a)
			if (`R_minus'<1 & `R_minus'<`R_full'){
			...
			forvalues k=1(1)`S'{
				...
				*simple correlation
				...
				quietly corr rankPMC_`k'`i' rankPMC_`k'`j'
				...
				* quietly reg   rankPMC_`k'`i'  nprod
				...
				display "i=`i' j=`j' and k=`k' Rfull=`R_full' R_minus=`R_' `i'_`j'  =  `temp'"		
...
file write myfile  "\documentclass{article}" _n  ///
				   ...
				   "\caption{Within rank correlation for product sales at 10 most popular destinations. Uses adjusted R2. (`country', `year') \label{AdjRankCorr_Select1st_SP}}" _n ///
